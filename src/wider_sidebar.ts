...
(function () {
  // Your code here...
  console.log(
    "This script widens the siderbar on the doc pages of python.org."
  );

  // specify how much wider
  const delta = 70;
  const bodywrapper = document.querySelector<HTMLDivElement>(".bodywrapper");

  // sidebar container, container actual sidebar and collapse button
  const sphinxsidebar =
    document.querySelector<HTMLDivElement>(".sphinxsidebar");

  // the actual list container, measures 13px less in width than sphinxsidebar
  const sphinxsidebarwrapper = document.querySelector<HTMLDivElement>(
  ...
  if (
    ...
    const bodywrapperOriginalMarginLeft = parseInt(
      getComputedStyle(bodywrapper).marginLeft
    );
    const sphinxsidebarOriginalWidth = parseInt(
      getComputedStyle(sphinxsidebar).width
    );
    const sphinxsidebarwrapperOriginalWidth = parseInt(
      getComputedStyle(sphinxsidebarwrapper).width
    );
